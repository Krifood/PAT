...
bool RecordParser::parseFile(const QString& path, QVector<Series>& outSeries, QString& errorMessage) const {
  ...
  outSeries.clear();
  outSeries.resize(static_cast<int>(format_.signalFormats.size()));
  ...
  for (int recordIndex = 0; recordIndex < recordCount; ++recordIndex) {
    ...
    for (int s = 0; s < static_cast<int>(format_.signalFormats.size()); ++s) {
      const auto& sig = format_.signalFormats[s];
      const int sz = typeSize(sig.valueType);
      if (sig.byteOffset + sz > format_.recordSize) {
      ...
      outSeries[s].samples.append(QPointF(recordIndex, value));
    }
  }
